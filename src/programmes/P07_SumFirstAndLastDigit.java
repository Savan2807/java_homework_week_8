...
public class P07_SumFirstAndLastDigit {
    public static int sumFirstAndLastDigit(int number) {
        if (number < 0){
        return -1;}// Return -1 for negative numbers

        int lastDigit = number % 10; // Extracting the last digit of the number

        while (number >= 10) // Finding the first digit of the number using a loop
        {number /= 10;}

        int firstDigit = number;
       return firstDigit + lastDigit; // Returning the sum of the first and last digits
